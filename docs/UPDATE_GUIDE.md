...
## 6. 楽天アフィリエイトHTMLを入れる場合

1. 楽天アフィリエイトで広告HTMLを作成します。
2. `data/affiliate_config.json` を開きます。
3. `enabled` を `true` にします。
4. `rakutenHtml` に、楽天が生成したHTMLをそのまま追加します。
5. 楽天側が生成した `href` URLは変更しません。
6. `python tools/build_site.py --csv data/survey_data.csv --template src/index.template.html --out index.html` を実行します。
